...
// Для моніторингу WiFi з'єднання
static uint32_t lastWifiCheckTime = 0;
static bool wifiWarningShown = false;
...
int audioPlayerLoop() {
    if (playerState == PLAYER_PLAYING) {
        // Періодична перевірка якості WiFi з'єднання (кожні 10 секунд)
        uint32_t currentTime = millis();
        if (currentTime - lastWifiCheckTime > 10000) {
            lastWifiCheckTime = currentTime;
            
            // Перевіряємо якість WiFi з'єднання
            if (appWifiIsConnected()) {
                int32_t rssi = appWifiGetRSSI();
                
                // Показуємо попередження про слабкий сигнал кожні 30 секунд
                if (rssi < WIFI_MIN_RSSI_THRESHOLD && !wifiWarningShown) {
                    Serial.printf("[Audio] WARNING: Weak WiFi signal (%d dBm). Audio dropouts possible.\n", rssi);
                    wifiWarningShown = true;
                } else if (rssi >= WIFI_MIN_RSSI_THRESHOLD) {
                    wifiWarningShown = false;  // Скидаємо попередження
                }
                
                // Якщо сигнал дуже слабкий, показуємо детальну інформацію
                if (rssi < -80) {
                    Serial.printf("[Audio] WiFi RSSI: %d dBm (POOR)\n", rssi);
                } else if (rssi < -70) {
                    Serial.printf("[Audio] WiFi RSSI: %d dBm (MODERATE)\n", rssi);
                } else {
                    Serial.printf("[Audio] WiFi RSSI: %d dBm (GOOD)\n", rssi);
                }
            }
        }
